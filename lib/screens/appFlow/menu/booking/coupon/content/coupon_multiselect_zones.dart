...
class CouponMultiselectZones extends StatelessWidget {
  CreateCouponProvider provider;
  CouponMultiselectZones({super.key, required this.provider});
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        provider.selectedCouponZones.isNotEmpty ?
        const Padding(
        ...
        Padding(
          ...
          child: Text(
            provider.selectedCouponZones.map((e) => e.name).join(", "),
          ),
        ),
      ],
    );
  ...
  void showZoneMultiSelectDialog(BuildContext context, List<DiscountComponent> items, CreateCouponProvider provider) {
    showDialog(
      context: context,
                ...
                child: ListView(
                  children: items.map((zone) {
                    return CheckboxListTile(
                      ...
                      value: provider.selectedCouponZones.any((selected) => selected.id == zone.id),
