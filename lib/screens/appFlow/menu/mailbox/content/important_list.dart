...
class ImportantList extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ChangeNotifierProvider(
      create: (_) => MailBoxProvider("important"),
      child: Consumer<MailBoxProvider>(
          builder: (context, provider, _) {
            final importantList = provider.mailInboxListResponse?.data?.mailbox;
            return Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          Row(
                            ...
                            children: [
                              const Text("Important", style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold),),
                              Text("Total : ${importantList?.length ?? ''}", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey)),
                            ],
                          ),
                          const SizedBox(height: 14,),
                          Container(width: MediaQuery.of(context).size.width, height: 1, color: Colors.grey,),
                          importantList?.isNotEmpty == true ?
                          ListView.builder(
                              shrinkWrap: true,
                              physics: const NeverScrollableScrollPhysics(),
                              itemCount: importantList?.length ?? 0,
                              itemBuilder: (BuildContext context, int index){
                                final data = importantList?[index];
                                return InkWell(
                                  onTap: (){
                                    ...
                                    children: [
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          ...
                                          InkWell(
                                              onTap : (){
                                                provider.updateStatus(context, "important", [data?.id ?? 0], 'starred', data?.isStarred == 0 ? 1 : 0 );
                                              },
                                          ...
                                          InkWell(
                                              onTap : (){
                                                provider.updateStatus(context, "important", [data?.id ?? 0], 'important', data?.isImportant == 0 ? 1 : 0 );
                                              },
                                          ...
                                          const SizedBox(width: 08,),
                                          InkWell(
                                              onTap: ()=> provider.moveToTrash(context, "important", [data?.id ?? 0]),
